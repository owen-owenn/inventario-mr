...
<head>
    ...
    <style>
        ...
        .brand-container { display: grid; grid-template-columns: repeat(2, 1fr); gap: 25px; margin-bottom: 30px; }
        ...
        .brand-btn { border: none; border-radius: 25px; cursor: pointer; font-weight: 900; font-size: 1.8rem; text-transform: uppercase; min-height: 160px; display: flex; align-items: center; justify-content: center; text-align: center; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 8px 15px rgba(0,0,0,0.15); color: white; text-shadow: 2px 2px 5px rgba(0,0,0,0.4); }
        .brand-btn:hover { transform: translateY(-8px) scale(1.05); box-shadow: 0 15px 25px rgba(0,0,0,0.3); }
        .btn-volver { background: #e2e8f0; color: #334155; border: none; padding: 15px 20px; border-radius: 15px; font-weight: 900; font-size: 1.2rem; cursor: pointer; margin-bottom: 25px; width: 100%; text-align: left; transition: 0.2s; display: flex; align-items: center; gap: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
        .btn-volver:hover { background: #cbd5e1; transform: translateX(-5px); }

        #grid-equipos { display: grid !important; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)) !important; gap: 20px !important; align-items: start !important; }
        @media (min-width: 1100px) { #grid-equipos { grid-template-columns: repeat(4, 1fr) !important; } }

        .card-modelo { background: white !important; border-radius: 16px !important; padding: 20px !important; box-shadow: 0 4px 15px rgba(0,0,0,0.05) !important; display: flex !important; flex-direction: column !important; justify-content: space-between !important; margin-bottom: 0 !important; }
...
<body>
    ...
    <button style="background-color: #8e44ad; color: white; border: none; padding: 15px 20px; border-radius: 30px; font-weight: bold; cursor: pointer; position: fixed; bottom: 150px; right: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); font-size: 1.1rem; z-index: 100; transition: 0.3s;" onclick="abrirModalMarca()">🏷️ Nueva Marca</button>
    ...
    <div id="modal-agregar" class="modal-oculto">
        <div class="modal-contenido">
            ...
            <!-- EL SELECT AHORA SE LLENARÁ SOLO DESDE LA BASE DE DATOS -->
    ...
    <!-- MODAL AGREGAR MARCA -->
    <div id="modal-marca" class="modal-oculto">
        <div class="modal-contenido">
            <span class="cerrar-modal" onclick="cerrarModalMarca()">&times;</span>
            <h2 style="text-align: center; color: #8e44ad; margin-top: 0; font-weight: 800;">Nueva Marca</h2>
            <input type="text" id="n-nueva-marca" placeholder="Nombre (ej. APPLE)" class="input-form" style="text-transform: uppercase;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 20px; margin-top: 10px;">
                <label style="font-weight: bold; color: #555;">Color del botón:</label>
                <input type="color" id="n-color-marca" value="#3498db" style="width: 100%; height: 50px; border: none; cursor: pointer; border-radius: 8px;">
            </div>
            <button onclick="guardarNuevaMarca()" class="btn-guardar" style="background-color: #8e44ad;">💾 Guardar Marca</button>
        </div>
    </div>
